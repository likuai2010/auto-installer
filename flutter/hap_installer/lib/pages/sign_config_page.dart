    ...
    return Consumer<EcoViewModel>(builder: (_, model, child){
        return ListView(children: [
          ...
          ListTile(title: Text("p12"), subtitle: Text("证书密钥，用于创建csr,cer,p7b等文件"), onTap: (){
            ...
            builder: (_)=> EditConfigDialog(name: "p12", value: model.signConfig?.keystoreFile??"", onChange: (p0) => model.signConfig?.keystoreFile = p0)
          );
          }, child: Text("更换")),),
          ListTile(title: Text("csr"), subtitle: Text("通过p12创建, 用于申请cer证书"), onTap: (){
            ...
            showDialog(
              context: context,
               builder: (_)=> EditConfigDialog(name: "csr", value: model.signConfig?.csrPath??"", onChange: (p0) => model.signConfig?.csrPath = p0)
            );
          }, child: Text("更换")),),
          ListTile(title: Text("keyAlias"), subtitle: Text("密钥别名"),onTap: (){
             ...
             showDialog(context: context, builder: (_)=> EditConfigDialog(name: "keyAlias", value: model.signConfig?.keyAlias??"", onChange: (p0) => model.signConfig?.keyAlias = p0));
          }, child: Text("更换")),),
          ListTile(title: Text("keyPwd"), subtitle: Text("密钥密码"), onTap: (){
              toask(context, "已复制到剪贴板");
              Clipboard.setData(ClipboardData(text: "${model.signConfig?.keystorePwd  }"));
          }, trailing: TextButton(onPressed: (){
            showDialog(context: context, builder: (_)=> EditConfigDialog(name: "keystorePwd", value: model.signConfig?.keystorePwd??"", onChange: (p0) => model.signConfig?.keyAlias = p0));
          }, child: Text("更换")),),
        ],);
    });
  }
...
showConfigEditAlert(EcoViewModel model, BuildContext context, String name){
  ...
  switch(name){
    case "p12": value = config?.keystoreFile ?? "";
    case "csr": value = config?.csrPath ?? "";
    case "keyAlias": value = config?.keyAlias ?? "";
    case "keystorePwd": value = config?.keystorePwd ?? "";
  }
 showDialog(context: context, builder: (_)=> EditConfigDialog(name: name, value: value, onChange: (p0){
    switch(name){
      case "p12": config?.keystoreFile = p0;
      case "csr": config?.csrPath = p0;
      case "keyAlias": config?.keyAlias = p0;
      case "keystorePwd": config?.keystorePwd = p0;
    }
...
class EditConfigDialog extends StatelessWidget {
  ...
  EditConfigDialog({super.key, required this.name, required value, this.onChange}){
