...
class CmdService {
    ...
    async unpackHap(hapFilePath, outPath) {
        const mode = getFileExtension(hapFilePath)
        const path = `--${mode}-path`
        let javaPath = this.JavaHome + "/bin/java"
        let unpackTool = this.SdkHome + "/lib/app_unpacking_tool.jar"
        let cmd = `${javaPath} -jar ${unpackTool} --mode ${mode} ${path} "${hapFilePath}" --out-path "${outPath}" --force true`
