...
class EcoService {
    ...
    getAcl(moduleJson) {
        if (!moduleJson || !moduleJson.module || !moduleJson.module.requestPermissions) {
            console.warn("not found requestPermissions", moduleJson)
            return []
        }
        let pers = moduleJson.module.requestPermissions.map(d => d.name)
        const set2 = new Set(pers);
        const intersection = this.aclList.filter(value => set2.has(value));
        console.warn("found acl", intersection)
        return intersection
    }

    createProfile(name, certId, packageName = "com.xiaobai.testgo", deviceIds = [], moduleJson = {}) {
